...
function createInitialForm(customerName: string): BookingForm {
  return {
...
export default function BookingForm({
  ...
  const [form, setForm] = useState<BookingForm>(() =>
    createInitialForm(customerName)
  );
 ...
 function updateOrderAmount(
  ...
  function applySavedAddress(
    ...
    if (destination === "pickup") {
      setForm((current) => ({
        ...current,
        sender_name: savedAddress.contact_name,
        sender_phone: savedAddress.phone,
        pickup_address: savedAddress.address,
      }));
    ...
    setForm((current) => ({
      ...current,
      receiver_name: savedAddress.contact_name,
      receiver_phone: savedAddress.phone,
      dropoff_address: savedAddress.address,
    }));
  ...
  return;
    ...
    try {
      ...
      setCompletedAmount(Number(result.pricing?.total_amount || 0));
      setForm(createInitialForm(customerName));
      setPickupPoint(null);
              ...
              >
                  ...
                  {savedAddresses.length > 0 && (
                    <label className="mb-5 block">
                      ...
                      <select
                        defaultValue=""
                        onChange={(event) =>
                          applySavedAddress(event.target.value, "pickup")
                        }
                        className="w-full rounded-2xl border border-blue-200 bg-white px-4 py-3.5 font-bold text-blue-800 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100"
                      >
                ...
                <section className="rounded-3xl border border-sky-100 bg-gradient-to-br from-white to-sky-50/60 p-5 md:p-7">
                  ...
                  {savedAddresses.length > 0 && (
                    <label className="mb-5 block">
                      <span className="mb-2 block text-sm font-bold text-slate-700">
                        Use saved drop-off address
                      </span>
                      <select
                        defaultValue=""
                        onChange={(event) =>
                          applySavedAddress(event.target.value, "dropoff")
                        }
                        className="w-full rounded-2xl border border-sky-200 bg-white px-4 py-3.5 font-bold text-blue-800 outline-none focus:border-sky-500 focus:ring-4 focus:ring-sky-100"
                      >
                        <option value="">Select saved address</option>
                        {savedAddresses.map((address) => (
                          <option key={address.id} value={address.id}>
